...
export default function Nav() {
  ...
  return (
    <>
      ...
      {/* Rendered as a sibling, not a header descendant: Safari treats an
          ancestor's backdrop-filter as a containing block for position:fixed
          children, which collapsed this panel to the header's own height. */}
      <div
        className={`fixed inset-0 top-[73px] z-40 flex flex-col gap-1 bg-bg px-8 pt-6 transition-opacity duration-150 sm:hidden ${
      ...
      >
        {LINKS.map((link) => (
          <Link
            ...
            tabIndex={open ? 0 : -1}
          >
            <span className="font-display block text-4xl tracking-wide text-ink">
              {link.label}
            </span>
          </Link>
        ))}
